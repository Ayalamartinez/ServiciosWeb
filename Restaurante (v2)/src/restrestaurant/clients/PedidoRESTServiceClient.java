...
public class PedidoRESTServiceClient {
	...
	public static void main(String[] args) {
			...
			try {
				...
				Clientes cliente = webTarget.path("rest").path("getCliente/ronny.ayalab@gmail.com").request()
				...
				dp.setEstatus("1");
				dp.setPedido(pedido);
				dp.setCant(new Double(1));				
				Plato p = webTarget.path("rest").path("plato/getPlato/44" ).request()
				...
				dp.setEstatus("1");
				dp.setPedido(pedido);
				dp.setCant(new Double(0));				
				p = webTarget.path("rest").path("plato/getPlato/45" ).request()
						.accept(MediaType.APPLICATION_JSON).get(Plato.class);	
				dp.setPlato(p);
				detalles.add(dp);
				...
				// Log
				 //Entity.entity(pedidoCompleto, MediaType.APPLICATION_JSON)
