...
public class PlatoRESTServiceClient {
	...
	public static void main(String[] args) {
			...
			try {
				String webServiceURI = "http://10.10.0.99:8080/Restaurante/";
				...
				Ingrediente i = webTarget.path("rest").path("ingrediente/getIngrediente/28" ).request()
						.accept(MediaType.APPLICATION_JSON).get(Ingrediente.class);				
				ingredientes.add(i);
				i = webTarget.path("rest").path("ingrediente/getIngrediente/29" ).request()
						.accept(MediaType.APPLICATION_JSON).get(Ingrediente.class);				
				ingredientes.add(i);				
				...
				*/
				...
				/*System.out.println(webTarget.path("rest").path("plato/updatePlato/" + idPlato).request()
						.accept(MediaType.APPLICATION_JSON).post(Entity.entity(plato, MediaType.APPLICATION_JSON)).bufferEntity());
				*/
				//Ejemplo de eliminar muchos registros entre los id 0 y 28
				
				for (int j = 0; j < 50; j++) {
					Long idPlato_n = new Long(j);
					System.out.println(webTarget.path("rest").path("plato/deletePlato/" + idPlato_n).request()
							.accept(MediaType.APPLICATION_JSON).post(Entity.entity(plato, MediaType.APPLICATION_JSON)).bufferEntity());
